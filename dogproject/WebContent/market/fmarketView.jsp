...
<div id="sub_content_wrap"  class="sub0202 inner">
	<div class="inner" style="margin-bottom:10em">
		...
		<article id="bo_v" style="width:100%">
		    <header>
		        <h2 id="bo_v_title">
		        	<span class="bo_v_tit">${market.ftitle}</span>
		    ...
		    <c:if test="${loginId == 'admin' || loginId == market.id}">
				<div class="btn_confirm write_div">
					...
					<a href="FmarketUpdateForm.do?seq=${param.seq}" id="btn_submit" class="btn_submit btn fr" style="margin-left: 10px;">글수정</a>
				</div>
			</c:if>
		    ...
		    <section id="bo_v_atc">
		       ...
		       <c:if test="${!market.filename.isEmpty() && market.filename != null}">
		        <div id="bo_v_img"><img src="./upload/img/${market.filename}"></div>
				</c:if>
				${market.fcontent}
		        <!-- 본문 내용 시작 { -->
		    	 </section>
		...
		<section id="bo_vc">
			<h2>댓글목록</h2>
			<c:if test="${mcomment.isEmpty()}">
				<p id="bo_vc_empty">등록된 댓글이 없습니다.</p>		
			</c:if>
			<c:forEach items="${mcomment}" var="co">
			<article>
			...
			<div class="cmt_contents">
			    ...
			    <ul class="bo_vc_act">
					<li><a href="FcommentDelete.do?seq=${co.seq}&m_seq=${param.seq}" class="btn_b03">삭제</a></li>
				</ul>
				</c:if>
			</div>
			</article>
		...
		<aside id="bo_vc_w" class="bo_vc_w">
		    <h2>댓글쓰기</h2>
			<form name="fviewcomment" id="fviewcomment" action="FmarketView.do?seq=${market.seq}" method="post">
			    <span class="sound_only">내용</span>
			        <textarea id="mcomment" name="mcomment" maxlength="10000" required class="required" title="내용" placeholder="댓글내용을 입력해주세요"></textarea>
