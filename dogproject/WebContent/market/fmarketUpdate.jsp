		  	...
		  	<form action="MarketUpdate.do" method="post" name="fwrite" id="fwrite" enctype="multipart/form-data">
			   <input type="hidden" name="seq" value="${market.seq}" >
				...
				<div class="bo_w_tit write_div bo_w_info write_div2">
					<span>제목</span>
            		<input type="text" name="ftitle" value="${market.ftitle}" id="ftitle" required="" class="frm_input full_input required" size="50" maxlength="255" placeholder="제목">
				</div>
				
				<div class="bo_w_tit write_div bo_w_info write_div2">
				     <span>판매상태</span>
<div style="margin-top:11.5px; padding-bottom:15px; font-size:0.9em">
							 &nbsp;&nbsp;<input type="radio" value ="판매중" name="fsselect" <c:if test="${fn:contains(market.fsselect,'판매중')}">checked="checked"</c:if> class="frm_input2"/> 판매중  &nbsp;&nbsp;
							 &nbsp;&nbsp;<input type="radio" value ="판매완료" name="fsselect" <c:if test="${fn:contains(market.fsselect,'판매완료')}">checked="checked"</c:if> class="frm_input2"/> 판매완료
					</div>
				</div>
				...
				<div class="bo_w_tit write_div bo_w_info write_div2">
				     <span>판매물품</span>
			<select name="fsell"> 
			<option value="fsell">==========선택=============</option>
			<option value="강아지  산책물품"
			<c:if test="${fn:contains(market.fsell,'강아지 산책물품')}">selected="selected"</c:if>> 강아지  산책물품</option>
			<option value="강아지 옷"
			<c:if test="${fn:contains(market.fsell,'강아지 옷')}">selected="selected"</c:if>> 강아지 옷  </option>
			<option value="강아지 생필품"
			<c:if test="${fn:contains(market.fsell,'강아지 생필품')}">selected="selected"</c:if>> 강아지 생필품 </option>
			<option value="강아지  간식"
			...
			<c:if test="${fn:contains(market.fsell,'기타')}">selected="selected"</c:if>> 기타 </option>
				...
				<div class="write_div  bo_w_info write_div2">
			        ...
			        <div style="" class="wr_content">
						<textarea id="fcontent" name="fcontent"  maxlength="65536" style="width: 100%; height: 300px;">${market.fcontent}</textarea>
		            </div>
			    </div>
